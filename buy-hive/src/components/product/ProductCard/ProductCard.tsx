...
export function ProductCard({ product, list = false }: ProductCardProps) {
  ...
  return (
    <Box
      className={`product-card ${list ? "list-product" : ""}`}
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
    >
      <Box className="product-image-wrap">
        <Box component="img" src={product.images[0]} alt="" className="product-image" />
      </Box>
      <Box className="product-details">
        <Typography className="product-name">{product.productName}</Typography>
        <Typography className="product-moq">MOQ: {product.displayMOQ}</Typography>
